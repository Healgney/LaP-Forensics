...
from model.lap import LaPForClassification
...
def parse_args():
    parser = argparse.ArgumentParser(description="LaP-Forensic Detection Training")

    # Model-specific settings
    parser.add_argument("--version", default="./checkpoints/lap_loc_exp/")
    ...
    parser.add_argument("--save_path", default="./checkpoint/lap_cls_train", type=str)    
    return parser.parse_args()

class LaPClsDataset(Dataset):
...
def initialize_model(args, tokenizer):
    ...
    model = LaPForClassification.from_pretrained(
        args.version, torch_dtype=torch.bfloat16 if args.precision == 'bf16' else torch.float16,
...
def main():
    ...
    # Prepare datasets
    train_dataset = LaPClsDataset(args, train=True)
    eval_dataset = LaPClsDataset(args, train=False)
