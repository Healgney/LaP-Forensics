...
def parse_args():
    ...
    parser.add_argument("--scama_loss_weight", default=0.0, type=float, help="Weight for SCAMA loss")
    # Dual-Stream Consistency settings (required by initialize_model)
    parser.add_argument("--use_consistency_stream", action="store_true", default=False)
...
def main():
    ...
    if lora_r > 0 and has_lora:
        print("LoRA keys detected. Setting up PEFT model...")
        lora_config = setup_lora_config(model, args)
        model = get_peft_model(model, lora_config)
        model.print_trainable_parameters()
    else:
        print("No LoRA keys detected or lora_r=0. Loading as standard model...")

    model.resize_token_embeddings(len(tokenizer))

    # Detect the structure of saved checkpoint and adapt key mapping
    sample_key = next(iter(state_dict.keys()))
    print(f"Sample key from checkpoint: {sample_key}")
    
    updated_state_dict = {}
    for key, value in state_dict.items():
        new_key = key
        # Handle double prefix (always reduce to single first)
        if new_key.startswith("base_model.model.base_model.model."):
            new_key = new_key.replace("base_model.model.base_model.model.", "base_model.model.")
        
        # If we are NOT using LoRA (standard model), we need to remove the PEFT prefix
        if not has_lora and new_key.startswith("base_model.model."):
            new_key = new_key.replace("base_model.model.", "")
        
        # If we ARE using LoRA, we need to ensure the PEFT prefix exists
        if has_lora and args.lora_r > 0 and not new_key.startswith("base_model.model."):
