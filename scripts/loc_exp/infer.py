...
def parse_args():
    parser = argparse.ArgumentParser(description="LaP-Forensic Inference")

    parser.add_argument("--hf_model_path", required=True, help="The directory to your lap ckpt")
    ...
    # Dual-Stream Consistency options
    parser.add_argument("--use_consistency_stream", action="store_true", help="Enable dual-stream with VAE consistency features")
    parser.add_argument("--vae_path", default="stabilityai/sd-vae-ft-mse", type=str,
                        help="Path or HuggingFace ID for the VAE used in the consistency stream")
    parser.add_argument("--consistency_encoder_type", default="clip", choices=["clip", "cnn"],
                        help="Encoder type for residual map features")
...
if __name__ == "__main__":
    ...
    # Only initialize distributed mode if using multiple GPUs
    if args.world_size > 1:
        init_distributed_mode(args)
    else:
        args.local_rank = 0  # Set default for single GPU
    ...
    model = LaPForCausalLM.from_pretrained(
        ...
        use_consistency_stream=args.use_consistency_stream,
        vae_path=args.vae_path,
        consistency_encoder_type=args.consistency_encoder_type,
        **kwargs,
    )
    # Update model config
    ...
    # Use dual-stream prompt if consistency stream is enabled
    if getattr(args, 'use_consistency_stream', False):
        instruction = '''The image is accompanied by a consistency analysis visualization. This consistency map highlights regions where the image deviates from natural reconstruction patterns - synthetic or AI-generated content typically shows higher deviation (brighter areas) in the consistency map due to the double encoding effect.

Please provide a detailed analysis of artifacts in this photo by examining:
1. **Visual artifacts**: Analyze the original image for physical artifacts (optical issues, violations of physical laws, spatial/perspective errors), structural artifacts (deformed objects, asymmetry, distorted text), and distortion artifacts (color/texture anomalies, noise/blur, material misrepresentation).
2. **Consistency artifacts**: Pay special attention to regions that show high deviation in the consistency map, as these often indicate AI-generated or manipulated content.
...
Output with interleaved segmentation masks for the corresponding parts of the answer.'''
    ...
    # Support both distributed and single GPU mode
    if args.world_size > 1:
        ...
        # Single GPU mode - no distributed sampler needed
        dataloader = DataLoader(dataset, batch_size=args.batch_size_per_gpu, num_workers=2,
