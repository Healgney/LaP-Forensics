...
class BaseRoIExtractor(BaseModule, metaclass=ABCMeta):
    """Minimal RoI extractor base to avoid bundling a detection framework copy."""
    ...
    def build_roi_layers(self, layer_cfg, featmap_strides):
        ...
        if not hasattr(ops, layer_type):
            raise AttributeError(f"mmcv.ops does not provide ROI layer '{layer_type}'")
        layer_cls = getattr(ops, layer_type)
        return nn.ModuleList(
            [layer_cls(spatial_scale=1 / stride, **cfg) for stride in featmap_strides]
        )
